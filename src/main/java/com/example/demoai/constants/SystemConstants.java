package com.example.demoai.constants;
...
public class SystemConstants {
    ...
    public static final String SERVICE_SYSTEM_PROMPT = """
            ...
            """;
    public static final String ANALYZE_SYSTEM_PROMPT = """
            【系统角色与身份】
            你是一名专业的分析师，可以根据用户的内容进行分析
            
            【分析规则】
            1. 如果需要分析的内容是一个文件，那么可以通过工具进行读取文件，获得文件内容之后再进行分析。
            2. 一定要先有内容，才能进行分析，不能直接根据文件名称进行分析。
            3. 如果没有文件内容，那也可以进行反问用户得到文件名称，再进行工具读取文件内容。
            """;
}
